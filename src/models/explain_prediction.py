...
model_info, con = restore_model('./models/FB15K/1906131339/model_info.json')
con.restore_tensorflow()

# Import lib files (IMPORTANT!)
...
with click.progressbar(list(test2id_df.iterrows())) as bar:
    for row in bar:
        # eh_id, et_id, r_id = [2956, 7611, 153]
        eh_id, et_id, r_id = row[1] # (UNCOMMENT)
        # res_id = [et_id]
        ...
        for tail_cand in tail_cands:
            head_marks = con.predict_head_entity(t=tail_cand, r=r_id, k=5)
